...
src_install() {
    ...
    mkdir -p "${D}etc/env.d"
	echo "CONFIG_PROTECT_MASK=\"${EPREFIX}/etc/ca-certificates.conf\"" > "${D}etc/env.d/98ca-certificates"
	#doenvd 98ca-certificates
}

pkg_postinst() {
	if [ -d "${EPREFIX}/usr/local/share/ca-certificates" ] ; then
		# if the user has local certs, we need to rebuild again
		# to include their stuff in the db.
		# However it's too overzealous when the user has custom certs in place.
		# --fresh is to clean up dangling symlinks
		"${EPREFIX}"/usr/sbin/update-ca-certificates --root "${EPREFIX}"
	fi

	local c badcerts=0
	for c in $(find -L "${EPREFIX}/etc/ssl/certs/" -type l) ; do
	...
	if [ $badcerts -eq 1 ]; then
		...
		ewarn "find -L ${EPREFIX}/etc/ssl/certs/ -type l -exec rm {} +"
	fi
}
