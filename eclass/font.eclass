...
font_cleanup_dirs() {
	...
	find -L "${EPREFIX}"/usr/share/fonts/ -type d -print0 | while read -d $'\0' d; do
		candidate=false
		otherfile=false
		for f in "${d}"/*; do
			generated=false
...
font_pkg_postinst() {
	...
	find "${EPREFIX}"/usr/share/fonts/ -type f '!' -perm 0644 -print0 \
	...
	if [[ -n ${FONT_CONF[@]} ]]; then
		...
		elog
		for conffile in "${FONT_CONF[@]}"; do
			if [[ -e "${EPREFIX}"/etc/fonts/conf.avail/$(basename ${conffile}) ]]; then
