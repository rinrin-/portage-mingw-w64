...
src_unpack() {
	...
	sed -i \
		-e "/SSL_CMD=\/usr\/bin\/openssl/s:=.*:=${EPREFIX}/usr/bin/openssl:" \
		"${WORKDIR}"/c_rehash \
	|| die
}
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${P}-mingw64-filecase.patch
	epatch_user #332661
...
src_install() {
	...
	mkdir -p ${D}etc/env.d
    echo "OPENSSL_CONF=${EPREFIX}/etc/ssl/openssl.cnf" >>"${D}etc/env.d/00openssl"
}
...
pkg_postinst() {
	ebegin "Running 'c_rehash ${EPREFIX}/etc/ssl/certs/' to rebuild hashes #333069"
	export OPENSSL_CONF=${EPREFIX}/etc/ssl/openssl.cnf
	c_rehash "${EPREFIX}/etc/ssl/certs" >/dev/null
	eend $?
